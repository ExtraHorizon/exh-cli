...
describe('exh dispatchers init', () => {
    ...
    expect(dispatcherFileAfter).toStrictEqual(
      {
        ...
        $schema: 'https://swagger.extrahorizon.com/cli/1.13.4/config-json-schemas/Dispatchers.json',
      }
    );
  });
  ...
  it('Throws an error when providing an invalid Dispatcher file path', async () => {
    ...
    const fileContentWithoutDispatchersArray = { $schema: 'https://swagger.extrahorizon.com/cli/1.13.4/config-json-schemas/Dispatchers.json' };
