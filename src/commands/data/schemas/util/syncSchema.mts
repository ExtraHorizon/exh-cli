...
/* using the CommonJS version of lodash through a default export */
const { pick, difference, omit, differenceBy, transform, isEqual, differenceWith } = lodash;
 ...
 */
  async #syncRootAttributes() {
    const diff = deepDiff(
      pick(this.target, 'description', 'defaultLimit', 'maximumLimit', 'createMode', 'readMode', 'updateMode', 'deleteMode', 'groupSyncMode'),
      pick(this.current, 'description', 'defaultLimit', 'maximumLimit', 'createMode', 'readMode', 'updateMode', 'deleteMode', 'groupSyncMode')
    );
 ...
 */
  async #syncProperties() {
    ...
    const incorrectProperties: any = deepDiff(
      omit(this.target.properties, missingProperties),
      omit(this.current.properties, excessProperties)
    );
 ...
 */
  async #updateStatuses() {
    ...
    // add missing
    const missingStatuses = difference(
    ...
    const incorrectStatuses: any = deepDiff(
      omit(this.target.statuses, missingStatuses),
 ...
 */
  async #syncCreationTransition() {
    ...
    if (!isEqual(this.current.creationTransition, this.target.creationTransition)) {
 ...
 */
  async #syncTransitions() {
    ...
    // evaluate missing
    const missingTransitions: any = differenceBy(
    ...
    // evaluate excess
    const excessTransitions: any = differenceBy(
      this.current.transitions,
      this.target.transitions,
      'name'
    );
    ...
    const compareTransitions = (targetTransition: any, currentTransition: any) => isEqual(
      targetTransition,
      omit(currentTransition, ['id'])
    ); // (ignore auto-generated id in comparison)

    // evaluate incorrect
    const incorrectTransitions: any = differenceWith(
      // targets, omit missing
      differenceBy(this.target.transitions, missingTransitions, 'name'),
      // current, omit excess
      differenceBy(this.current.transitions, excessTransitions, 'name'),
    ...
    const findTransitionByName = (name: string) => this.current.transitions.find(transition => transition.name === name);
 ...
 */
  async #pruneStatuses() {
    ...
    const excessStatuses = difference(
      Object.keys(this.current.statuses),
      Object.keys(this.target.statuses)
    );
  ...
  async #syncIndexes() {
    ...
    for (const newIdx of this.target.indexes) {
      ...
      for (const oldIdx of filteredCurrentIndexes) {
        if (isEqual(omit(oldIdx.idx, ['name', 'id', 'system']), omit(newIdx, ['name']))) {
          found = true;
          oldIdx.marked = true;
          break;
        }
...
function deepDiff(object: any, other: any) {
  ...
  return transform(object, (result, value, key) => {
    ...
    if (!isEqual(value, other[key])) {
      ...
      result[key] = isObject(value) && isObject(other[key]) ? deepDiff(value, other[key]) : value;
    }
  });
}
